 ...
 */
public class IntegerIntervalEdit extends BContainer {
	 ...
	 * 
	 */
	public IntegerIntervalEdit(EditAnnotationWindow w, Field f) {
		super();
		...
		textField = new BTextField(Integer.toString(editor.getInt(field)));
		textField.setPreferredSize(40, -1);

		int min = field.getAnnotation(Editable.class).intMinValue();
		int max = field.getAnnotation(Editable.class).intMaxValue();
		final int step = field.getAnnotation(Editable.class).intStepValue();
		

		slider = new BSlider(Orientation.HORIZONTAL,
				min/step, max/step, 
				editor.getInt(field)/step);

		slider.getModel().addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(ChangeEvent event) {
				logger.info("changed");
				editor.setInt(field,slider.getModel().getValue()*step);
				slider.getModel().setValue(editor.getInt(field)/step);
				textField.setText(Integer.toString(editor.getInt(field)));
			}
		});
		...
		textField.addFocusListener(new FocusListener() {
			...
			public void focusLost(FocusEvent event) {
				editor.setInt(field, textField.getText());
				slider.getModel().setValue(editor.getInt(field)/step);
				textField.setText(Integer.toString(editor.getInt(field)));
			}
