...
public class Ellipse extends AbstractFigure {
	public Ellipse(int id, int x0, int y0, int x1, int y1, Color color,
			boolean fill, int thickness) {
		super(id, x0, y0, x1, y1, color, fill, thickness);
	}

	public Ellipse(int id, Point p0, Point p1, Color color, boolean fill,
			int thickness) {
	...
	public String toString() {
		return "oval¤" + p0.x + "¤" + p0.y + "¤" + p1.x + "¤" + p1.y + "¤"
	 ...
	 */
	public static Ellipse newBBOval(int id, String params) {
		ArrayList<String> paramsList = AbstractFigure.toShapeParams(params);
		...
		return new Ellipse(id, p0, p1, c, fill, thick);
	}
}
